        ...
        file.messages = file.messages.filter((message) => {
          const foundMessage = diffFile.messages.find((diffMessage) => {
            return messageEqualFields.reduce((a, b) => {
              ...
              if (result){
                let masterMessage = diffMessage[b],
                    currentMessage = message[b];

                if (b === 'source'){
                  masterMessage = diffMessage[b].replace(/\s/g, '');
                  currentMessage = message[b].replace(/\s/g, '').replace(/["']/g, (_, index) => masterMessage[index]);
                }

                result = masterMessage === currentMessage;
              }
              return result;
            }, true);
          });
